...
#       make the three column format eventfile [onsetTime, Durration, Magnitude]
        ...
        reactionTime3Col = trialByTrial[(trialByTrial.OptValue  == trialByTrial.OptValue) & (trialByTrial.Run  == run)][['ReactionTime','ReactionTime']]
...
#       Name and open the destinations for event files
        ...
        reactionTimeDir= safe_open_w(data_dir + '/Models/' + modelName + '/EventFiles/' + subjectID + '/RUN' + str(run) + '/ReactionTime.run00'+ str(run) +'.txt')
...
#       write each 3-column event file as a tab dilimited csv
        ...
        reactionTime3Col.to_csv(reactionTimeDir, sep ='\t', header = False)
