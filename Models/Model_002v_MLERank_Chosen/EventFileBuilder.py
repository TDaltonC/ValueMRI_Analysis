...
def chosen_value(row):
    if row['Choice'] == 1:
        chosenValue = row.OptValue
    elif row['Choice'] == 2:
        chosenValue = row.FixedValue
    else: chosenValue = np.nan
    return chosenValue
...
modelName = "Model_002v_MLERank_Chosen"
...
#   load the trial by trial data for this subject
    ...
    trialByTrial['chosenValue'] = trialByTrial.apply(chosen_value, 1)
    trialByTrial = trialByTrial[trialByTrial['chosenValue'] == trialByTrial['chosenValue']]
...
#       make the three column format eventfile [onsetTime, Durration, Magnitude]
        ...
        controlValue3Col = trialByTrial[(trialByTrial.Opt1Type == 1) & (trialByTrial.Run  == run) & (trialByTrial.OptValue  == trialByTrial.OptValue)][['ReactionTime','chosenValue']]
        ...
        scalingValue3Col = trialByTrial[(trialByTrial.Opt1Type == 2) & (trialByTrial.Run  == run) & (trialByTrial.OptValue  == trialByTrial.OptValue)][['ReactionTime','chosenValue']]
        ...
        bundlingValue3Col = trialByTrial[(trialByTrial.Opt1Type == 3) & (trialByTrial.Run  == run) & (trialByTrial.OptValue  == trialByTrial.OptValue)][['ReactionTime','chosenValue']]
...
#       De-Mean the parametric pregressors
        controlValue3Col['chosenValue'] = controlValue3Col['chosenValue'] - controlValue3Col['chosenValue'].mean()
        # controlDifficulty3Col['chosenValueDiff'] = controlDifficulty3Col['chosenValueDiff'] - controlDifficulty3Col['chosenValueDiff'].mean()

        scalingValue3Col['chosenValue'] = scalingValue3Col['chosenValue'] - scalingValue3Col['chosenValue'].mean()
        # scalingDifficulty3Col['chosenValueDiff'] = scalingDifficulty3Col['chosenValueDiff'] - scalingDifficulty3Col['chosenValueDiff'].mean()
        
        bundlingValue3Col['chosenValue'] = bundlingValue3Col['chosenValue'] - bundlingValue3Col['chosenValue'].mean()
        # bundlingDifficulty3Col['chosenValueDiff'] = bundlingDifficulty3Col['chosenValueDiff'] - bundlingDifficulty3Col['chosenValueDiff'].mean()
