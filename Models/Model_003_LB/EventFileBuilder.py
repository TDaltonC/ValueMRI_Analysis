...
# The subjects will consider the lease valuable of any of those three that is greater then the fixedOption value
...
def valueBuilder(row):
    if np.nanmax([row.ComOptValue, row.Item1Value, row.Item2Value]) > row.FixedValue:
        values = np.array([row.ComOptValue, row.Item1Value, row.Item2Value])
        return np.nanmin(values[values>row.FixedValue])
    else:
